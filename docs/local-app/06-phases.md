...
| 1.4 | Jira client port (`backend/app/jira/client.py`): auth, paginate, 429/503 retry honouring `Retry-After`, custom-field discovery (Story Points, Sprint, Epic Link, Initiative Link with parent-walk fallback). Sprint-field dual-shape parser (legacy GH string vs. modern object). Unit tests with `respx` fixtures captured via `scripts/capture_jira_fixtures.py`. | 1.5d |
