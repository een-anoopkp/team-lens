...
Pipeline (per docs/local-app/03-sync-engine.md):
 ...
 10. Snapshot diff hook (sync/snapshots.update_snapshots)
 11. Project freeze hook (sync/projects.run_freeze_job)
 12. Insight anomaly evaluation (best-effort, never aborts)
 13. Update sync_runs row (status=success + counts)
"""
...
from app.sync import comments, issues, projects, snapshots, sprints
...
class SyncRunner:
    ...
    async def _execute(
        ...
        async with JiraClient(
            ...
            await self._ctx.fields.refresh(jira)
            ...
            # 10. Snapshot diff hook
            await snapshots.update_snapshots(
            ...
            # 11. Project freeze hook
            await projects.run_freeze_job(self._ctx.session_factory)

            # 12. Insight anomaly evaluation — never aborts the sync.
            try:
