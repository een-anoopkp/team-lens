/**
 * team-lens: Epic Risk — CSV export.
 *
 * exportCsvs() is wired to the Sheet menu. It serialises the user-facing
 * tabs plus a small meta row (drawn from the latest RunLog entry) and
 * opens a modal dialog with one-click download buttons. The user drops
 * the downloaded files into ~/dashboards/epic-risk/ where the local
 * HTML page reads them.
 *
 * Tabs that are published:
 *   - epics.csv               ← Epics
 *   - epic_sprint_history.csv ← EpicSprintHistory
 *   - meta.csv                ← synthesised from the newest RunLog row
 *
 * Raw Tickets is intentionally not exported (design — lives only in the
 * Sheet for ad-hoc pivots).
 */

function exportCsvs() {
  const ss = SpreadsheetApp.getActive();
  const epicsCsv = sheetToCsv_(ss.getSheetByName('Epics'));
  const historyCsv = sheetToCsv_(ss.getSheetByName('EpicSprintHistory'));
  const metaCsv = buildMetaCsv_(ss.getSheetByName('RunLog'));

  const files = {
    'epics.csv': epicsCsv,
    'epic_sprint_history.csv': historyCsv,
    'meta.csv': metaCsv
  };
...
function buildMetaCsv_(runLog) {
  const header = ['last_run_iso', 'last_run_status', 'rows_epics', 'rows_tickets', 'error'];
...
function buildExportDialog_(files) {
  ...
  const body =
    ...
    '<div class="hint">Drop the files into <code>~/dashboards/epic-risk/</code>, ' +
  ...
  return HtmlService.createHtmlOutput(body).setWidth(420).setHeight(280);
}
