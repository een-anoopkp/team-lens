/**
 * team-lens: Sprint Health — scheduled triggers.
 *
 * installDailyTrigger() registers a time-based trigger that fires
 * refreshFromJira() once per day at 07:00 in the project timezone
 * (see appsscript.json). removeDailyTrigger() tears down any existing
 * triggers for refreshFromJira so you can cleanly reset.
 *
 * Daily cadence matters for the snapshot-diff approach: longer gaps
 * collapse multiple mid-sprint SP edits into a single delta.
 */
